...
├── include/                # 퍼블릭 헤더 예약 폴더 (.gitkeep 보관, 미사용)
├── src/                    # 전체 소스 및 헤더 (단일 디렉터리 구조)
│   ├── Calculator.h / Calculator.cpp
│   ├── InputParser.h / InputParser.cpp
│   ├── AppController.h / AppController.cpp
│   ├── types.h
│   ├── main.cpp            # C++ 진입점
│   └── main.c              # legacy placeholder (미사용)
├── tests/                  # 단위·통합 테스트 소스 (Google Test)
│   ├── unit/
│   └── integration/
...
| 2    | SPL2-BUILD-0002                                         | calc_lib 정적 라이브러리 빌드 | `cmake --build build --target calc_lib`         | `src/*.cpp`, `src/*.h`              | `build/libcalc_lib.a`                            | 공통      | CLI 빌드의 선행 조건             |
...
| v1.1 | 2026-04-12 | SUP10-CR-0001 반영 — §1.2 디렉터리 구조 및 §2.1 SPL2-BUILD-0002 입력 경로를 실제 구조(`src/`)로 정정 | ASPICE SUP-10 전문가 |
